...
@section('title', 'Buat Kontrak Baru - MyPH')
...
@section('content')
        ...
        <div class="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6">
                <div class="flex items-center gap-4 mb-2">
                    ...
                    <h2 class="text-2xl font-heading font-bold text-slate-900 dark:text-slate-100">Buat Kontrak Baru</h2>
                </div>
                <p class="text-slate-600 dark:text-slate-400 font-body ml-14">Buat kontrak baru yang akan ditampilkan di
                    dashboard</p>
            </div>
                ...
                <form action="{{ route('contracts.store') }}" method="POST" id="contractForm">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="w-full px-4 py-3 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-blue-500"
                            placeholder="Dengan ini, setiap peserta Padamu HIMATIKA ITS 2024 menyetujui hal-hal berikut:">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div id="rulesContainer" class="space-y-3 mb-4">
                            @if (old('rules'))
                                ...
                                @endforeach
                            @else
                                <!-- Default 5 rules as template -->
                                <div class="rule-item flex gap-2">
                                    <span
                                        class="flex-shrink-0 w-8 h-10 bg-gradient-to-br from-blue-600 to-cyan-500 text-white rounded-lg flex items-center justify-center font-bold text-sm rule-number">1</span>
    ...
    <script>
        let ruleCounter = document.querySelectorAll('.rule-item').length;
