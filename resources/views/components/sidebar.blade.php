...
<aside id="sidebar"
    ...
    <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto relative z-10">
        ...
        @if (Auth::user()->isMember())
            <a href="{{ route('upload') }}"
                class="group flex items-center justify-center px-4 py-3 rounded-xl transition-all duration-200 relative overflow-hidden {{ request()->routeIs('upload') ? 'bg-white/20 shadow-lg' : 'hover:bg-white/10' }}"
